...
|---|---|
| Desktop (≥ 1200px) | Kanban в 4 колонки, боковая панель |
| Tablet (768–1200px) | Kanban в 2 колонки, боковая панель |
| Mobile (< 768px) | Kanban в 1 колонку, drawer-меню сверху |

---

## Дизайн-система

Приложение использует CSS Custom Properties (`var(--*)`) для поддержки тем. Основные токены:

```css
--accent          /* Фиолетовый акцент #6c47ff */
--bg              /* Фон страницы */
--bg-card         /* Фон карточек */
--text / --text-2 /* Основной и вторичный текст */
--border          /* Цвет рамок */
--radius          /* Скругление 12px */
--shadow          /* Тень карточек */
```

Шрифты: **Space Grotesk** (заголовки) + **Inter** (основной текст).
